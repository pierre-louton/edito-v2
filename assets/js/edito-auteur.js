...
(function ($) {
  ...
  const Editor = {
    ...
    init() {
      ...
      this.bindUpload();
      this.bindActions();
      // bindCategoryPreview() supprimé : le select unique est remplacé
      // par des checkboxes gérées directement dans editor.php
    },
     ...
     * ------------------------------------------------------------------ */
    bindActions() {
      $('#btn-update').on('click',     () => this.save('publish'));
    ...
    save(status) {
      ...
      // #ce-cat-ids contient les IDs séparés par des virgules,
      // mis à jour en temps réel par le JS des checkboxes dans editor.php.
      const catIdsRaw = $('#ce-cat-ids').val() || '';
      ...
      // ── Date de publication ───────────────────────────────────────────
      // Le champ datetime-local renvoie 'YYYY-MM-DDTHH:MM',
      // on normalise en 'YYYY-MM-DD HH:MM' pour le PHP.
      const postDateRaw = $('#ce-post-date').val() || '';
      const postDate    = postDateRaw.replace('T', ' ');
      ...
      if (catIds.length === 0) {
        this.showToast('Veuillez sélectionner au moins une catégorie.', 'error');
        // Scroller vers la card catégories pour guider l'utilisateur
        document.getElementById('edito-cats-checkboxes')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
        return;
      }

      $('#ce-content-hidden').val(content);

      const $btn = status === 'draft'   ? $('#btn-save-draft')
                 : status === 'publish' ? $('#btn-update')
                 : $('#btn-submit');
      ...
      const data = {
        ...
        title,
        content,
        // ── MODIFIÉ : cat_ids[] remplace category ──────────────────────
        // jQuery sérialise les tableaux en cat_ids[]=1&cat_ids[]=2,
        // ce que PHP reçoit dans $_POST['cat_ids'] comme tableau.
        cat_ids:   catIds,
        ...
        if (res.success) {
          ...
          if ('pending' === status) {
            ...
            const labels = {
              ...
              publish: 'Mettre à jour',
              pending: 'Soumettre pour validation',
            };
            $btn.text(labels[status] || 'Enregistrer');
          }
        } else {
